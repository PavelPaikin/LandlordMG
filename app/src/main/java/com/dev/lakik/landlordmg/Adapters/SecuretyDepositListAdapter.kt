...
class SecuretyDepositListAdapter(context: Context, var items: ArrayList<Lease.securityDeposit>) : BaseAdapter() {
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View? {
        ...
        if (convertView == null) {
            view = this.mInflator.inflate(R.layout.lv_securety_deposit_item_edit, parent, false)
        ...
        vh.btnRemove.setOnClickListener(View.OnClickListener { view ->
            val item = view.tag as Lease.securityDeposit
            items.remove(item)
        ...
        vh.etName.setOnFocusChangeListener { v, hasFocus ->
            if(!hasFocus) {
                val item = v.tag as Lease.securityDeposit
        ...
        vh.etAmount.setOnFocusChangeListener { v, hasFocus ->
            if(!hasFocus) {
                var item = v.tag as Lease.securityDeposit
                val editText = v as EditText
    ...
    public fun addEmptyRow(){
        items.add(Lease.securityDeposit())
